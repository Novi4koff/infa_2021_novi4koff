...
def flower(x, y, size, angle):
    ...
    surface = pygame.Surface((650,700))
    surface.set_colorkey([0, 0, 0])
    angle1=angle*((np.pi)/180)
    
    x1=x*np.cos(angle1)+y*np.sin(angle1)
    y1=x*np.sin(angle1)+y*np.cos(angle1)
    print(x1, y1)
    
    ellipse(surface, White, [(x - 4*size, y + 2*size), (8*size, 4*size)]) #Нижний лепесток
    ellipse(surface, White, [(x - 4*size, y - 6*size), (8*size, 4*size)]) #Верхний лепесток
    ellipse(surface, White, [(x - 9*size, y - 4*size), (8*size, 4*size)]) #Верхний правый диагональный лепесток
    ellipse(surface, White, [(x - 9*size, y), (8*size, 4*size)]) #Нижний правый диагональный лепесток
    ellipse(surface, White, [(x + size, y - 4*size), (8*size, 4*size)]) #Верхний левый диагональный лепесток
    ellipse(surface, White, [(x + size, y), (8*size, 4*size)]) #Нижний левый диагональный лепесток
    ellipse(surface, Yellow, [(x - 4*size, y - 2*size), (8*size, 4*size)]) #Сердцевина
    surface2 = pygame.transform.rotate(surface, angle)
    
    screen.blit(surface2, (x-x1, y-y1))
'''
def flower(x, y, size, angle):
    #Эта функция рисует цветок с жёлтой сердцевиной и белыми лепестками с центром в точке (x,y) и размером, в size раз больще единичного размера
    surface = pygame.Surface((17*size,17*size)) # генерация поверхности, которую мы будем вращать
    surface.set_colorkey([0, 0, 0]) #делаем поверхность прозрачной

    ellipse(surface, White, [(4*size, 2*size), (8*size, 4*size)]) #Нижний лепесток
    ellipse(surface, White, [(4*size, 6*size), (8*size, 4*size)]) #Верхний лепесток
    ellipse(surface, White, [(9*size, 4*size), (8*size, 4*size)]) #Верхний правый диагональный лепесток
    ellipse(surface, White, [(9*size, 0), (8*size, 4*size)]) #Нижний правый диагональный лепесток
    ellipse(surface, White, [(size, 4*size), (8*size, 4*size)]) #Верхний левый диагональный лепесток
    ellipse(surface, White, [(size, 0), (8*size, 4*size)]) #Нижний левый диагональный лепесток
    ellipse(surface, Yellow, [(4*size, 2*size), (8*size, 4*size)]) #Сердцевина
    surface2 = pygame.transform.rotate(surface, angle)
    
    screen.blit(surface2, (x - 4*size, y + 2*size)) #координаты цветка (поверхности)
...
flower(540, 490, 1, 78)
flower(542, 510, 1, 24)
flower(190, 600, 1.5, 98)
flower(210, 625, 1.5, 45)
flower(215, 590, 1.5, 156)
flower(595, 645, 1.5, 76)
flower(615, 660, 1.5, 14)
